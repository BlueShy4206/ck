...
/*
        ...
        */
        ...
        if (!empty($caller->status) && $caller->status == AUTH_EXPIRED) {
            ...
            $status = '<b>The username or password is not correct！</b>'."\n";
        } else if (!empty ($caller->status) && $caller->status == AUTH_SECURITY_BREACH) {
...
//        echo '<form method="post" action="'.$caller->server['PHP_SELF'].'" onSubmit="'.$loginOnClick.'">'."\n";
        ...
        echo '<table WIDTH="300" HEIGHT="80" border="0" cellpadding="0" cellspacing="0" summary="login form" align="right">'."\n";
        echo '<tr>'."\n";
        ...
        //onClick=" '.$loginOnClick.' "
        echo '    <td colspan="2" align="right"><input value="Login" id="doLogin" name="doLogin" type="submit" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>'."\n";
        echo '</tr>'."\n";
        echo '</table>'."\n";
        // Might be a good idea to make the variable name variable 
