...
class HTML_Javascript_Convert {
	 ...
	 *        	if true, the JS var will be global
	...
	function convertVar($var, $varname, $global = false) {
		...
		switch ($var_type) {
			...
			case 'NULL' :
				return HTML_Javascript_Convert::convertNull ( $varname, $global );
			
			default :
				return HTML_Javascript_Convert::raiseError ( HTML_JAVASCRIPT_ERROR_CONVERT_INVVAR, __FUNCTION__ . ':' . $var_type );
		}
	}
	...
	function raiseError($code, $str = '') {
		...
		switch ($code) {
			case HTML_JAVASCRIPT_CONVERT_ERROR_INVVAR :
				return PEAR::raiseError ( 'Invalid variable:' . $str, $code );
			
			default :
				return PEAR::raiseError ( 'Unknown Error:' . $str, $code );
		}
	}
	 ...
	 *        	the variable name to declare
	 * @param boolean $global
	 *        	set to true to make the JS variable global
	 ...
	 *        	if true, the JS var will be global
	 * @param int $level
	 *        	Not public, used for recursive calls
	 * @return mixed a PEAR_Error if no script was started
	...
	function convertArray($arr, $varname, $global = false, $level = 0) {
		...
		if (is_array ( $arr )) {
			...
			return HTML_Javascript_Convert::raiseError ( HTML_JAVASCRIPT_CONVERT_ERROR_INVVAR, __FUNCTION__ . ':' . gettype ( $arr ) );
		}
	}
	 ...
	 *        	the variable name to declare
	 * @param boolean $new
	 *        	if true, the JS var will be set
