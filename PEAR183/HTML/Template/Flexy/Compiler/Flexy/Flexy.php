...
class HTML_Template_Flexy_Compiler_Flexy_Flexy {
	...
	function toJavascriptToString($element) {
		...
		foreach ( $element->attributes as $k => $v ) {
			...
			$v = substr ( $v, 1, - 1 );
			$ret .= $this->compiler->appendPhp ( '$__tmp = HTML_Javascript_Convert::convertVar(' . $element->toVar ( $v ) . ',\'' . $prefix . $k . '\',true);' . 'echo (is_object($__tmp) && is_a($__tmp,"PEAR_Error")) ? ("<pre>".print_r($__tmp,true)."</pre>") : $__tmp;' );
			$ret .= $this->compiler->appendHTML ( "\n" );
		}
	...
	function toJSONToString($element) {
		...
		if (! function_exists ( 'json_encode' )) {
			$ret = $this->compiler->appendPhp ( 'require_once "Services/JSON.php"; $_json = new Services_JSON();' );
		}
		...
		foreach ( $element->attributes as $k => $v ) {
			...
			$v = substr ( $v, 1, - 1 );
			if (function_exists ( 'json_encode' )) {
				$ret .= $this->compiler->appendPhp ( 'echo "var ' . $k . '=" . json_encode(' . $element->toVar ( $v ) . ') . ";\n";' );
				$ret .= $this->compiler->appendHTML ( "\n" );
				continue;
			}
			$ret .= $this->compiler->appendPhp ( 'echo "var ' . $k . '=" . $_json->encode(' . $element->toVar ( $v ) . ') . ";\n";' );
	...
	function includeToString($element) {
		...
		switch (true) {
			...
			default :
				...
				return HTML_Template_Flexy::raiseError ( ' Flexy:Include SRC needs a string or variable/method as value. ' . " Error on Line {$element->line} &lt;{$element->tag}&gt;", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
		}
