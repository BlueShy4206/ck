...
class HTML_Template_Flexy_Compiler_Flexy_Tag {
	 ...
	 *
	 * @param
	 *        	string Namespace handler for element.
	 * @param
	 *        	object HTML_Template_Flexy_Compiler
	 *        	
	 *        	
	...
	function toString($element) {
		...
		if (isset ( $_HTML_TEMPLATE_FLEXY ['currentOptions'] ['output.block'] ) && ($_HTML_TEMPLATE_FLEXY ['currentOptions'] ['output.block'] == $element->getAttribute ( 'ID' ))) {
		...
		foreach ( $element->attributes as $k => $v ) {
			...
			if (strtoupper ( $k ) == 'FLEXY:RAW') {
				if (! is_array ( $v ) || ! isset ( $v [1] ) || ! is_object ( $v [1] )) {
					return $this->_raiseErrorWithPositionAndTag ( 'flexy:raw only accepts a variable or method call as an argument, eg.' . ' flexy:raw="{somevalue}" you provided something else.' . 
	...
	function toStringChildren(&$element, &$ret) {
		...
		if (((! empty ( $element->tag )) && ($element->tag == 'SCRIPT')) || ((! empty ( $element->oTag )) && ($element->oTag == 'SCRIPT'))) {
	...
	function parseAttributeForeach() {
		...
		if (strpos ( $first, '(' ) !== false) {
			while ( strpos ( $first, ')' ) === false ) {
				if (! count ( $foreachTokens )) {
					return $this->_raiseErrorWithPositionAndTag ( "Missing Closer on functin call: An flexy:foreach attribute was found. flexy:foreach=&quot;$foreach&quot;<BR>
                        the syntax is  &lt;sometag flexy:foreach=&quot;onarray,withvariable[,withanothervar] &gt;<BR>", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
				}
				$first .= ',' . array_shift ( $foreachTokens );
			}
		}
		...
		$foreachObj = $this->element->factory ( 'Foreach', $foreachTokens, $this->element->line );
		// failed = probably not enough variables..
		
		if ($foreachObj === false) {
			
			return $this->_raiseErrorWithPositionAndTag ( "Missing Arguments: An flexy:foreach attribute was found. flexy:foreach=&quot;$foreach&quot;<BR>
                 the syntax is  &lt;sometag flexy:foreach=&quot;onarray,withvariable[,withanothervar] &gt;<BR>", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
		}
		...
		if (! $this->element->close) {
			...
			if ($this->element->getAttribute ( '/' ) === false) {
				...
				return $this->_raiseErrorWithPositionAndTag ( "A flexy:foreach attribute was found without a corresponding &lt;/{$this->element->tag} tag", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
			}
			// it's an xhtml tag!
			$this->element->postfix = array (
					$this->element->factory ( "End", '', $this->element->line ) 
			);
		} else {
			$this->element->close->postfix = array (
					$this->element->factory ( "End", '', $this->element->line ) 
			);
		}
	...
	function parseAttributeIf() {
		...
		if (isset ( $this->element->hasForeach )) {
			return $this->_raiseErrorWithPositionAndTag ( "You may not use FOREACH and IF tags in the same tag", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
		}
		...
		if (! preg_match ( '/^[_A-Z][A-Z0-9_]*(\[[0-9]+\])?((\[|%5B)[A-Z0-9_]+(\]|%5D))*' . '(\.[_A-Z][A-Z0-9_]*((\[|%5B)[A-Z0-9_]+(\]|%5D))*)*(\\([^)]*\))?$/i', $if )) {
			return $this->_raiseErrorWithPositionAndTag ( "IF tags only accept simple object.variable or object.method() values. {$if}", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
		}
		...
		if (substr ( $if, - 1 ) == ')') {
			...
			$ifObj = $this->element->factory ( 'Method', array (
					'if:' . $ifnegative . substr ( $if, 0, strpos ( $if, '(' ) ),
					$args_clean 
			), $this->element->line );
		} else {
		...
		if (! $this->element->close) {
			...
			if ($this->element->getAttribute ( '/' ) !== false) {
				...
				return $this->_raiseErrorWithPositionAndTag ( "An flexy:if attribute was found in &lt;{$this->element->name} tag without a corresponding &lt;/{$this->element->name} tag", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
			}
		} else {
	...
	function _parseTags() {
		...
		if (in_array ( strtolower ( $tag ), array (
				'menulist',
				'textbox',
				'checkbox' 
		) )) {
		...
		if (($this->element->getAttribute ( 'NAME' ) === false) && ($this->element->getAttribute ( 'ID' ) === false)) {
			return false;
		}
	...
	function getElementPhp($id, $mergeWithName = false, $varsOnly = false) {
		...
		if (! $id) {
			...
			return $this->_raiseErrorWithPositionAndTag ( "Dynamic tags require an ID value", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
		}
		
		// dont mix and match..
		if (($this->element->getAttribute ( 'FLEXY:IF' ) !== false) || ($this->element->getAttribute ( 'FLEXY:FOREACH' ) !== false)) {
			return $this->_raiseErrorWithPositionAndTag ( " You can not mix flexy:if= or flexy:foreach= with dynamic form elements  " . " (turn off tag to element code with flexyIgnore=0, use flexy:ignore=&quot;yes&quot; in the tag" . " or put the conditional outside in a span tag", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
		}
		
		if ((strtolower ( $this->element->getAttribute ( 'TYPE' ) ) == 'checkbox') && (substr ( $this->element->getAttribute ( 'NAME' ), - 2 ) == '[]')) {
		...
		if (isset ( $_HTML_TEMPLATE_FLEXY ['elements'] [$id] )) {
			...
			return $this->_raiseErrorWithPositionAndTag ( "The Dynamic tag Name '$id' has already been used previously by  tag &lt;{$_HTML_TEMPLATE_FLEXY['elements'][$id]->tag}&gt;", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
		}
			...
			for($i = 1; $i < count ( $ar ) - 1; $i ++) {
				switch (true) {
					...
					case is_string ( $ar [$i] ) :
						...
						break;
					default :
						return $this->_raiseErrorWithPositionAndTag ( "unsupported type found in attribute, use flexy:ignore to prevent parsing or remove it. " . print_r ( $this->element, true ), null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
				}
			}
			$str = trim ( ltrim ( $str, '.' ) );
	...
	function parseTagScript() {
		...
		if ($lang == "PHP") {
			...
			return $this->_raiseErrorWithPositionAndTag ( 'PHP code found in script (script)', HTML_TEMPLATE_FLEXY_ERROR_SYNTAX, HTML_TEMPLATE_FLEXY_ERROR_RETURN );
		}
		return false;
	}
	/**
	...
	function parseTagInput() {
		...
		if (in_array ( strtoupper ( $this->element->getAttribute ( 'TYPE' ) ), array (
				'SUBMIT',
				'BUTTON',
				'INPUT',
				'' 
		...
		if (isset ( $this->element->ucAttributes ['FLEXY:RAW'] )) {
			return $this->_raiseErrorWithPositionAndTag ( "Flexy:raw can only be used with flexy:ignore, to prevent conversion of html " . "elements to flexy elements", null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
		}
		...
		if (! isset ( $_HTML_TEMPLATE_FLEXY ['elements'] [$id] )) {
			$_HTML_TEMPLATE_FLEXY ['elements'] [$id] = new HTML_Template_Flexy_Element ( "input", array (
					'type' => 'radio' 
			) );
		}
	...
	function parseTagTextArea() {
		return $this->compiler->appendPhp ( $this->getElementPhp ( $this->element->getAttribute ( 'NAME' ) ) );
	}
	/**
	...
	function parseTagForm() {
		...
		return $this->compiler->appendPhp ( $prefix . 'echo $this->elements[\'' . $id . '\']->toHtmlnoClose();' . $suffix ) . $this->element->compileChildren ( $this->compiler ) . $this->compiler->appendHtml ( "</{$copy->oTag}>" );
	}
	...
	function parseTagLabel() {
		...
		return $this->compiler->appendPhp ( 'echo "<label for=\"' . $for . '\">";' . 'if (!empty($this->elements[\'' . $for . '\']->label)) ' . ' { echo htmlspecialchars($this->elements[\'' . $for . '\']->label); } else { ?>' . htmlspecialchars ( $ret ) . '<? } ' . 'echo "</label>";' );
	}
	...
	function reWriteURL($which) {
		...
		foreach ( $bits as $bit ) {
			...
			$parts = explode ( ':', $bit );
			if (! isset ( $parts [1] )) {
				return $this->_raiseErrorWithPositionAndTag ( 'HTML_Template_Flexy: url_rewrite syntax incorrect' . print_r ( array (
						$bits,
						$bits 
				), true ), null, HTML_TEMPLATE_FLEXY_ERROR_DIE );
			}
	 ...
	 *
	 * @param
	 *        	object token to convert into a element.
	 ...
	 *
	 * @param
	 *        	HTML_Template_Flexy_Token element to check.
	 ...
	 * @param
	 *        	type (see HTML_Template_Flexy::raiseError())
	 * @param
	 *        	boolean isFatal.
	 *        	
	 * @access private
