...
class HTML_Template_Flexy_Token {
	 ...
	 *        	string Token type
	 * @param
	 *        	mixed Initialization settings for token
	 * @param
	 *        	int line that the token is defined.
	 *        	
	 *        	
	 ...
	 *
	 * @param
	 *        	object Tokenizer to run.. - Theoretically other Tokenizers could be done for email,rtf etc.
	 *        	
	 * @access public
	...
	function buildTokens($tokenizer) {
		...
		$_HTML_TEMPLATE_FLEXY_TOKEN ['tokens'] = array (
				new HTML_Template_Flexy_Token () 
		);
		...
		while ( $t = $tokenizer->yylex () ) {
			...
			if ($t == HTML_TEMPLATE_FLEXY_TOKEN_ERROR) {
				...
				// print_r($tokenizer);
				$err = "<PRE>" . $tokenizer->error . "\n" . htmlspecialchars ( substr ( $tokenizer->yy_buffer, 0, $tokenizer->yy_buffer_end ) ) . "<font color='red'>" . htmlspecialchars ( substr ( $tokenizer->yy_buffer, $tokenizer->yy_buffer_end, 100 ) ) . ".......</font></PRE>";
				
				return HTML_Template_Flexy::raiseError ( 'HTML_Template_Flexy::Syntax error in ".
                    "Template line:' . $tokenizer->yyline . $err, HTML_TEMPLATE_FLEXY_ERROR_SYNTAX, HTML_TEMPLATE_FLEXY_ERROR_RETURN );
			}
			...
			if ($tokenizer->value->token == 'Php') {
				if (! $GLOBALS ['_HTML_TEMPLATE_FLEXY'] ['currentOptions'] ['allowPHP']) {
					return HTML_Template_Flexy::raiseError ( 'PHP code found in script (Token)', HTML_TEMPLATE_FLEXY_ERROR_SYNTAX, HTML_TEMPLATE_FLEXY_ERROR_RETURN );
				}
		...
		for($i = 1; $i < $total; $i ++) {
			...
			if ($res [$i]->tag {0} == '/') { // it's a close tag..
				...
				for($s = $ssc; $s > - 1; $s --) {
					if (! isset ( $stack [$s] )) {
						echo "MISSED STACK? $s<BR><PRE>";
						print_r ( $stack );
						exit ();
					}
					if (! isset ( $_HTML_TEMPLATE_FLEXY_TOKEN ['tokens'] [$stack [$s]] )) {
						echo "STACKED BAD OFFSET : {$stack[$s]}<BR><PRE>";
						print_r ( $stack );
						exit ();
					}
	 ...
	 * 
	 * @param
	 *        	int id of node to add children to.
	 *        	
	 * @access public
	 * @static
	 *
