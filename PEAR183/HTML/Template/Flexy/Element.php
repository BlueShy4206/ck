...
class HTML_Template_Flexy_Element {
	 ...
	 * optionally set the text of this label.
	 * This permits
	 ...
	 * 
	 * @param HTML_Element $from
	 *        	override settings from another element.
	 * @access public
	 */
				...
				foreach ( $this->children as $i => $child ) {
					...
					if ($child->tag == 'optgroup') {
						foreach ( $this->children [$i]->children as $ii => $child ) {
							...
							if (isset ( $child->attributes ['value'] ) && in_array ( ( string ) $child->attributes ['value'], $value )) {
								$this->children [$i]->children [$ii]->attributes ['selected'] = isset ( $this->attributes ['flexy:xhtml'] ) ? 'selected' : true;
								continue;
							}
							if (isset ( $child->attributes ['value'] ) && isset ( $this->children [$i]->children [$ii]->attributes ['selected'] )) {
					...
					if (isset ( $child->attributes ['value'] ) && in_array ( ( string ) $child->attributes ['value'], $value )) {
						...
						$this->children [$i]->attributes ['selected'] = isset ( $this->attributes ['flexy:xhtml'] ) ? 'selected' : true;
						;
						continue;
					}
					// no value attribute try and use the contents.
					if (! isset ( $child->attributes ['value'] ) && is_string ( $child->children [0] ) && in_array ( ( string ) $child->children [0], $value )) {
						
						$this->children [$i]->attributes ['selected'] = isset ( $this->attributes ['flexy:xhtml'] ) ? 'selected' : true;
						continue;
					}
					
					if (isset ( $child->attributes ['value'] ) && isset ( $this->children [$i]->attributes ['selected'] )) {
	 ...
	 * @param HTML_Element $from
	 *        	override settings from another element.
	 * @param HTML_Element $noValue
	 *        	ignore the key part of the array
	 * @access public
	 */
	function setOptions($array, $noValue = false) {
		...
		foreach ( $array as $k => $v ) {
			if (is_array ( $v )) { // optgroup
				$child = new HTML_Template_Flexy_Element ( $namespace . 'optgroup', array (
						'label' => $k 
				) );
				foreach ( $v as $kk => $vv ) {
					$atts = array ();
					...
					$add->children = array (
							htmlspecialchars ( $vv, ENT_COMPAT, $charset ) 
					);
					$child->children [] = $add;
				}
				$this->children [] = $child;
			...
			$add = new HTML_Template_Flexy_Element ( $namespace . 'option', $atts );
			$add->children = array (
					htmlspecialchars ( $v, ENT_COMPAT, $charset ) 
			);
			$this->children [] = $add;
		}
	}
	...
	function toHtml($overlay = false) {
		...
		if (in_array ( strtoupper ( $tag ), array (
				"INPUT",
				"IMG",
				"LINK",
				"META",
	 ...
	 *
	 * @param HTML_Template_Flexy_Element $new
	 *        	(with data to replace/merge)
