...
class HTML_Template_Flexy_Translator {
	...
	var $options = array (
			...
			'targetLangs' => array (
					'fr' 
			), // the language the templates are being translated to.
	 ...
	 *
	 * @param
	 *        	array see options array in file.
	 * @return none
	 * @access public
	 */
	 ...
	 *        	(translate = en)
	 * @param array $_POST;
	 *        	(translate = en, en[{md5}] = translation)
	 *        	
	 * @return none
	 * @access public
	 */
			...
			foreach ( $words as $word ) {
				...
				if ($value == '') {
					...
					if (isset ( $old [addslashes ( $word )] )) {
						$trd->add ( $word, $page, array (
								$displayLang => $old [addslashes ( $word )] 
						) );
	 ...
	 *
	 * @param
	 *        	string subdirectory of templateDir or empty
	 * @return none
	 * @access public
	 */
	function compileAll($d = '') {
		...
		while ( ($name = readdir ( $dh )) !== false ) {
			...
			$this->words [$fname] = file_exists ( $x->getTextStringsFile ) ? unserialize ( file_get_contents ( $x->getTextStringsFile ) ) : array ();
		}
