...
class HTML_Crypt {
	...
	/**
	 * a preg expression for an <a href=mailto: ...
	 * tag
	 *
	 * @access public
	 * @var string
	 ...
	 *        	The text to encrypt
	 * @param int $offset
	 *        	The offset used to encrypt/decrypt
	 * @param boolean $JS
	 *        	If javascript shall be used on the client side
	 *        	
	 * @access public
	 */
	...
	function getScript() {
		...
		$script = '<script language="JavaScript" type="text/javascript">' . '/*<![CDATA[*/' . 'var a,s,n;' . 'function ' . $rnd . '(s){' . 'r="";' . 'for(i=0;i<s.length;i++){' . 'n=s.charCodeAt(i);' . 'if(n<128){' . 'n=n-' . $this->offset . ';' . 'if(n<32){' . 'n=127+(n-32);' . '}' . '}' . 'r+=String.fromCharCode(n);' . '}' . 'return r;' . '}' . 'a="' . str_replace ( '"', '\\"', $this->cryptString ) . '";' . 'document.write(' . $rnd . '(a));' . '//]]></script>';
	...
	function obEnd() {
		...
		$text = preg_replace_callback ( "/{$this->apreg}/", array (
				$this,
				'_fly' 
		), $text );
		ob_end_clean ();
