...
class HTML_Javascript {
	 ...
	 *
	 * @param integer $mode
	 *        	the chosen output mode, can be either
	...
	function raiseError($code) {
		...
		switch ($code) {
			...
			default :
				return PEAR::raiseError ( 'Unknown Error', HTML_JAVASCRIPT_ERROR_UNKNOWN );
				break;
		}
	 ...
	 *
	 * @param bool $defer
	 *        	whether to wait for the whole page to load
	...
	function startScript($defer = true) {
		$this->_started = true;
		$s = $defer ? 'defer="defer"' : '';
		$ret = "<script type=\"text/javascript\" " . $s . ">" . HTML_JAVASCRIPT_NL;
		return $ret;
	}
	 ...
	 *
	 * @param string $str
	 *        	the string returned from the calling function
	 ...
	 * @param boolean $var
	 *        	set to true if $str is a variable name
	 * @return mixed PEAR_Error if no script was started or the processed string
	 */
	function write($str, $var = false) {
		if ($var) {
			$ret = HTML_Javascript::_out ( 'document.writeln(' . $str . ')' . HTML_JAVASCRIPT_NL );
		} else {
			$ret = HTML_Javascript::_out ( 'document.writeln("' . HTML_Javascript_Convert::escapeString ( $str ) . '")' . HTML_JAVASCRIPT_NL );
		}
		return $ret;
	}
	...
	function writeLine($str, $var = false) {
		if ($var) {
			return HTML_Javascript::_out ( 'document.writeln(' . $str . '+"<br />")' . HTML_JAVASCRIPT_NL );
		}
		
		return HTML_Javascript::_out ( 'document.writeln("' . HTML_Javascript_Convert::escapeString ( $str ) . '"+"<br />")' . HTML_JAVASCRIPT_NL );
	}
	 ...
	 *
	 * @param string $assign
	 *        	the JS variable to assign the confirmation box to
	 * @param string $str
	 *        	the string that will appear in the confirmation box
	 * @param bool $var
	 *        	whether $str is a JS var or not
	...
	function confirm($str, $assign, $var = false) {
		if ($var) {
			...
			$confirm = 'confirm("' . HTML_Javascript_Convert::escapeString ( $str ) . '")' . HTML_JAVASCRIPT_NL;
		}
	 ...
	 *
	 * @param string $str
	 *        	the string that will appear in the prompt
	 * @param string $assign
	 *        	the JS var that the input will be assigned to
	 * @param string $default
	 *        	the default value
	 * @param string $var
	 *        	wether $str is a JS var or not
	...
	function prompt($str, $assign, $default = '', $var = false) {
		if ($var) {
			...
			$prompt = 'prompt("' . HTML_Javascript_Convert::escapeString ( $str ) . '", "' . $default . '")' . HTML_JAVASCRIPT_NL;
		}
	 ...
	 * @param mixed $attr
	 *        	an array containing the attributes for the new
	...
	function popup($assign, $file, $title, $width, $height, $attr, $top = 300, $left = 300) {
		...
		$ret = HTML_Javascript::_out ( $assign . "= window.open(" . "\"$file\", \"$title\"," . " \"width=$width, height=$height," . "resizable=$attr[0], scrollbars=$attr[1]," . " menubar=$attr[2], toolbar=$attr[3], status=$attr[4]," . " location=$attr[5], top=$attr[6], left=$attr[7]\")" . HTML_JAVASCRIPT_NL );
		return $ret;
	}
	 ...
	 * Inside the popup windows
	 * you can access the opener window with the opener var.
	 *
	 * @param string $assign
	 *        	the JS variable to assign the window to
	 * @param string $str
	 *        	the string that will appear in the new window
