...
class HTML_QuickForm_element extends HTML_Common {
	...
	function getFrozenHtml() {
		$value = $this->getValue ();
		return (strlen ( $value ) ? htmlspecialchars ( $value ) : '&nbsp;') . $this->_getPersistantData ();
	 ...
	 *
	 * @param
	 *        	HTML_QuickForm_Renderer renderer object
	 * @param
	...
	function _generateId() {
		...
		if (! $this->getAttribute ( 'id' )) {
			$this->updateAttributes ( array (
					'id' => 'qf_' . substr ( md5 ( microtime () . $idx ++ ), 0, 6 ) 
			) );
		}
