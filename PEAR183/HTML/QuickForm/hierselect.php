...
class HTML_QuickForm_hierselect extends HTML_QuickForm_group {
	 ...
	 *        	(optional)Use a string for one separator,
	 ...
	 *
	 * @param array $options
	 *        	Array of options defining each element
	 * @access public
	 * @return void
	 */
	...
	/**
	 * Sets the options for the first select element.
	 * Deprecated. setOptions() should be used.
	 *
	 * @param array $array
	...
	/**
	 * Sets the options for the second select element.
	 * Deprecated. setOptions() should be used.
	 ...
	 *
	 * @param array $value
	 *        	An array of 2 or more values, for the first,
...
JAVASCRIPT;
			...
			$this->_js .= "_hs_defaults['" . $this->_escapeString ( $this->getName () ) . "'] = " . $this->_convertArrayToJavascript ( $values ) . ";\n";
		}
		...
		if (! empty ( $onChange )) {
			$keys = array_keys ( $this->_elements );
			for($i = 0; $i < count ( $keys ) - 1; $i ++) {
				$this->_elements [$keys [$i]]->updateAttributes ( array (
						'onchange' => $onChange [$i] 
				) );
			}
		}
			...
			if ('addElement' == $event) {
				...
				if (strlen ( $onReset )) {
					if (strpos ( $onReset, '_hs_setupOnReset' )) {
						$caller->updateAttributes ( array (
								'onreset' => str_replace ( '_hs_setupOnReset(this, [', "_hs_setupOnReset(this, ['" . $this->_escapeString ( $this->getName () ) . "', ", $onReset ) 
						) );
					} else {
						$caller->updateAttributes ( array (
								'onreset' => "var temp = function() { {$onReset} } ; if (!temp()) { return false; } ; if (typeof _hs_setupOnReset != 'undefined') { return _hs_setupOnReset(this, ['" . $this->_escapeString ( $this->getName () ) . "']); } " 
						) );
					}
				} else {
					$caller->updateAttributes ( array (
							'onreset' => "if (typeof _hs_setupOnReset != 'undefined') { return _hs_setupOnReset(this, ['" . $this->_escapeString ( $this->getName () ) . "']); } " 
					) );
				}
			}
			return $ret;
	 ...
	 *        	array Options array
	 * @param
	 *        	int Depth within options array
	...
	function _prepareOptions($ary, $depth) {
		if (! is_array ( $ary )) {
			...
			$ret = array (
					'values' => array_keys ( $ary ),
					'texts' => array_values ( $ary ) 
			);
		} else {
			$ret = array ();
			foreach ( $ary as $k => $v ) {
	...
	function _encodeNameValue($name, $value) {
		return $this->_convertScalarToJavascript ( ( string ) $name ) . ':' . $this->_convertArrayToJavascript ( $value );
	}
