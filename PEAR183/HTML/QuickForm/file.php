...
class HTML_QuickForm_file extends HTML_QuickForm_input {
	 ...
	 * @param
	 *        	mixed (optional)Either a typical HTML attribute string
	...
	function _ruleIsUploadedFile($elementValue) {
		if ((isset ( $elementValue ['error'] ) && $elementValue ['error'] == 0) || (! empty ( $elementValue ['tmp_name'] ) && $elementValue ['tmp_name'] != 'none')) {
	 ...
	 *        	array Uploaded file info (from $_FILES)
	 * @param
	 *        	mixed Mime Type (can be an array of allowed types)
	 * @access private
