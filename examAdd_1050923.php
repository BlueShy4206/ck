...
// 	echo "cert_id=".$row_web_cert['cert_id']."<br>";
 ...
 $insertSQL = sprintf("INSERT INTO examinee (birthday, username, uname, eng_uname, sex, email, 
 		...
 		Department, pic_title, pic_name, `date`, id, contact, contact_ph, Qualify, status, cert_no, cert_id,examyear_id) 
 		VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,
 		%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",//, certificate
...
$query_web_allguide = sprintf("SELECT * FROM allguide Where up_no='EA2'",GetSQLValueString('%', "text"));
$web_allguide = mysql_query($query_web_allguide, $conn_web) or die(mysql_error());
$i=0;
...
<div id="main">
  ...
  <? if(strtotime($row_web_new['startday']) <= strtotime(date('Y-m-d')) && strtotime(date('Y-m-d')) <= strtotime($row_web_new['endday'])&& $row_web_new['status'] == '0'){?>
    ...
    <form id="form3" name="form3" method="post" enctype="multipart/form-data" action="<?php echo $editFormAction; ?>" >
      ...
      <table width="540" border="0" cellspacing="0" cellpadding="2">
        ...
        <!--	<td height="30" align="right" class="board_add"><span class="font_red">* </span>身份：</td>
         ...
         <tr>
          <td height="30" align="right" class="board_add"><span class="font_red">* </span>報考資格：<br>錄取順序一.<br>錄取順序二.<br>錄取順序三.<br>錄取順序四.<br>錄取順序五.</td>
           <td align="left" class="board_add">
          <label>本評量報考資格分為五類，其錄取順序如下：<br>
          	...
          	<input <?php if (!(strcmp($row_web_examinee2['cert_no'],"4"))) {echo "checked=\"checked\"";} ?> type="radio" name="id" id="radio3" value="4"  onclick="popIdMsg();" />
          	<?php echo $allguide_lot[3]['nm']."<br>"?>
          	<input <?php if (!(strcmp($row_web_examinee2['cert_no'],"5"))) {echo "checked=\"checked\"";} ?> type="radio" name="id" id="radio3" value="5"  onclick="popIdMsg();" />
          	<?php echo $allguide_lot[4]['nm']?>
          </label>
          </td>
         </tr>
        <tr>
         ...
         <label> 數學 </label>
          ...
          <label>社會 </label>
          <input type="checkbox" name="Subjects[]" id="Subjects[]" value="4" />
          <label>自然
          </label>
          (可複選)</td>
        </tr>
        ...
        <tr>
          ...
          <select name="school">
                ...
                <option value="10_國立新竹教育大學" <?php if (!(strcmp($row_web_examinee2['school'],"10_國立新竹教育大學"))) {echo "selected=\"selected\"";} ?> >國立新竹教育大學</option>
        ...
        <tr>
          ...
          <input type="text" name="Department" id="Department"  value="<?php echo $row_web_examinee2['Department']; ?>"/>(已畢業者請填最高學歷之就讀科系)
