(function(window, undefined) {
    ...
    var scriptId = "mdnkids_3813_11338_12004_1_script";
    ...
    function vce_url(c3, adid, wid, cid) {
      ...
      // append VCE          
      var url = "<script src='http://b.voicefive.com/c2/19172860/rs.js#c1=3&amp;c3=" + c3 + 
                "&amp;c4=" + adid + 
                "&amp;c5=" + cid + 
                "&amp;c6=&amp;c10=1&amp;c11=" + temp[0] + 
    ...
    // 5. global function: detect if element is in viewport
    function elementInViewPort(target, w) {

        // if the target does not exist 
        if (typeof (target) === typeof (undefined)) return true;
        ...
        // get view port offsets
        if (typeof (w.pageXOffset) == 'number') {
            // this works for all browsers except IE versions 8 and before
            win['pageXOffset'] = w.pageXOffset;
            win['pageYOffset'] = w.pageYOffset;
        } else {
            if (d.compatMode == 'CSS1Compat') {
                // for IE (or any browser) in Standards mode
                win['pageXOffset'] = d.documentElement.scrollLeft;
                win['pageYOffset'] = d.documentElement.scrollTop;
            } else {
                // for browsers in Quirks mode
                win['pageXOffset'] = d.body.scrollLeft,
                win['pageYOffset'] = d.body.scrollTop
            }
        }
        ...
        if (typeof (w.innerWidth) == 'number') {
            // most browsers
            win['innerWidth'] = w.innerWidth;
            win['innerHeight'] = w.innerHeight;
        } else if (d.documentElement &&
                   (d.documentElement.clientWidth || d.documentElement.clientHeight)) {
            // for IE 8
            win['innerWidth'] = d.documentElement.clientWidth;
            win['innerHeight'] = d.documentElement.clientHeight;
        } else if (d.body && (d.body.clientWidth || d.body.clientHeight)) {
            // for IE 9
            win['innerWidth'] = d.body.clientWidth;
            win['innerHeight'] = d.body.clientHeight;
        }

        // get offset and dimension of target
        ...
        while (target.offsetParent) {
            ...
            _left += target.offsetLeft;
        }

        // determine if target is in view port or not
        return (_top >= win['pageYOffset'] &&
                 _left >= win['pageXOffset'] &&
                 (_top + _height) <= (win['pageYOffset'] + win['innerHeight']) &&
                 (_left + _width) <= (win['pageXOffset'] + win['innerWidth']));
    }
    ...
    // player parameters
        ...
        volume = 0.0,
        adviewerArg = "&channelid=12004&wid=3813,8,24&PubID=lQYIGAg3QTM=",
        volume_on_by_default = false,
        auto_start = false,
        play_only_in_view_port = false;
    
    var pub = 'lQYIGAg3QTM=', cid = '12004', wid = '3813,8,24';;
    ...
    try {
        var notTakenCallBack = function () {
            var i;

            notTakenCallBack = function () {};

            // find publisher callback
            if(window.scupioexpandable && window.scupioexpandable.length) {
                for (i = window.scupioexpandable.length - 1; i >= 0; i--) {
                   if (window.scupioexpandable[i] && window.scupioexpandable[i].id === scriptId && typeof window.scupioexpandable[i].callback === 'function') {
                       window.scupioexpandable[i].callback();
                       window.scupioexpandable[i].callback = null;
                   }
                }
            }
        };
        ...
        if (waiting < 0) {
            ...
            if ( (typeof (jQQ) !== "undefined") && (typeof (VideoJS) !== "undefined") && requiredLibAreNotLoaded && (_s.response[id] !== null) ) {
                ...
                if ((typeof (_s.response[id].open) === "number") || _s.response[id].expandablehide) {
                    ...
                    var strVar = "";
                    strVar += "<div id=\"scupio-expandable" + id + "\" style=\"margin-left:auto;margin-right:auto;margin-top:0;margin-bottom:0;padding:0;border:1px solid #cfcfcf;vertical-align:baseline;background-color:#cfcfcf;width:950px;height:" + (_s.response[id].expandablehide ? "90" : "14") + "px;position:relative;z-index:65535;cursor:default;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;-o-user-select:none;user-select:none;\">";
                    ...
                    parameters.event = {
                        ...
                        on30s: function(mouse, in_view) {
                            // send trueview log
                            if (trueViewEvent && in_view) {
                        ...
                        onEnded: function(in_view, player_id, mouse) {
                            ...
                            if (_s.response[id].expandablehide) {
                                ...
                                jQQ("#expandable-inner" + id).stop(true, true).delay(3000).hide(0, function() {
                                    jQQ("#scupio-expandable" + id).stop(true, true).animate({
                                        height: "0px"
                                    }, function() {
                                        open = false;
                                ...
                                jQQ("#expandable-inner" + id).delay(600).show(0, function() {
                                    ...
                                    lid = setInterval(function() {
                                        ...
                                        if (typeof barbar !== "undefined" && barbar.ready) {
                                            ...
                                            (function poll_in_view(){
                                                var videoplayer = document.getElementById('scupio-expandable-player-hook' + id);
                                                if (videoplayer && elementInViewPort(videoplayer)) {
                                                    barbar.play = true;
                                                } else {
                                                    setTimeout(poll_in_view, 200);
                                                }
                                            }());
                                        }
