...
var commenturl ="";//"http://comment.epochtimes.com";
var comments;
var offset = 0;
var step = 1000;
var baseTime;
...
function replaceWrongEncoding()
{
  if($("form#commentForm")!=undefined && $("form#commentForm").attr("action") != undefined) 
  {
		$("form#commentForm").attr("action",$("form#commentForm").attr("action").replace("http://wwwga.epochtimes.com",""));
  }

  $("a[href*=wwwga]").each(function(){
      var href = $(this).attr("href");
	  //if(href != undefined) $(this).attr("href",href.replace("http://wwwga.epochtimes.com","").replace("encoding=gb","encoding="+tempencoding).replace("encoding=b5","encoding="+tempencoding));
	  if(href != undefined) $(this).attr("href",href.replace("encoding=gb","encoding="+tempencoding).replace("encoding=b5","encoding="+tempencoding));
  });
}
...
function handleNewComment()
{
  ...
  $("form#commentForm").find("input[name=submit]").before("<span name=\"captcha\"></span>");
  
  replaceWrongEncoding();
  

  if(badcaptcha == true) refreshcaptcha("span[name=captcha]");
  
  //$("form#commentForm").submit(function(event){
  //	 var c = $("input[name=captcha]").val();
  //	 if(c == undefined || c =="") event.preventDefault();
  // });
  
  $("textarea[name=commentContent]").keyup(function(){
      var c = $("input[name=captcha]").val();
	  if( c == undefined){
	       refreshcaptcha("span[name=captcha]");
		  //$("span[name=captcha]").html("<img style=\"margin-top:5px\" src=\"/services/index/captcha\"/><input name=\"captcha\" >");
	  }
  });
}
...
{
  //$(target).html(ac("請輸入認證碼")+":<img style=\"margin-top:5px\" src=\"/services/index/captcha?"+new Date().getTime()+"\"/><input name=\"captcha\" >");
}

function  diggArticle()
{
  $("a[name=diggarticle]").click(function(){
     url = $(this).attr("hrefa");
	 //alert(url);
	 $.get(commenturl+url,null,function(data){
	    if(data.status="ok") alert(data.message);
	 });
	 return;
	   $.ajax({
	       url:commenturl+url+"&callback=?",
	       dataType:"jsonp",
...
function replaceOldComment()
{
   var len = $("table.commenttable").length;
   
   if($("table.commenttable").length>0){
     if($("div[name=comments]").length<=0) $("table.commenttable").eq(0).before('<div name="comments" style="border:0px solid blue;margin:10px 50px 10px 50px;"></div>');
	 $("table.commenttable").remove();
   }
}

function handleComments()
{
   var matches = document.location.href.match(/\/([a-z|0-9]{2})\/\d+\/\d*\/\d+\/n(\d+)\.htm/);
...
//   $.getJSON(commenturl+"/services/ajax/getcomments?aid="+tempid+"&t=uc&encoding="+tempencoding+"&offset="+offset+"&number="+step+"&callback=?",null,function(data){
   $.getJSON(commenturl+"/services/ajax/getcomments?aid="+tempid+"&t=uc&encoding="+tempencoding+"&offset="+offset+"&number="+step,null,function(data){
      if(data.status == "ok"){
        offset += step;
        comments = data.comments;
	    baseTime = strToDateTime(comments.currenttime);//new Date(comments.currenttime.replace(" ","T"));
		
		populateChildren(tempid,tempencoding,0,null);
      }
   });
...
{
        var crs = comments.rows;//JSON.clone(comments.rows);
        $.each(crs,function(i,row){
		    if(row.pid == parentid)    {
...
function populateRow(row,id,ec,level)
{
   ...
   var html = $("<div name='row' style='margin-bottom:5px;margin-left:"+level*50+"px;'>"
		           +"<div style='height:30px;background-color:#EAF0F5;border:1px solid #4F8BB6;padding:3px;'>"
			          +"<span style='float:left'>"+ac("大紀元網友")+"</span><span style='float:right'>"+rowAge(row.time.replace(" ","T"))+"</span>"
		           +"</div>"
	               +"<div style='padding:3px;text-align:left'>"
		              //+ac(row.content.replace(/\r\n/g,"<br>").replace(/\\r\\n/g,"<br>").replace(/\n/g,"<br>"))
		              +row.content.replace(/\r\n/g,"<br>").replace(/\\r\\n/g,"<br>").replace(/\n/g,"<br>")
                           +"</div>"
	               +"<div style='padding:3px;height:30px;padding:3px;'>"
					   +"<span style='float:right'>"
						 +"【<a name='flagbad'  onclick='return false;' href='#'>"+ac("舉報")+"</a>】&nbsp;|&nbsp;"
						 +"【<a  name='digg'   onclick='return false;' href='#'>"+ac("推薦")+"</a>】&nbsp;|&nbsp;"
						 +"【<a name='reply'  onclick='return false;' href='#'>"+ac("跟評")+"</a>】"
					   +"</span>"
		           +"</div>"
	       +"</div>");
   ...
   $(html).find("a[name=reply]").click(function(){
       ...
       var win = $("<div name='input'>"
                       +"<textarea style='width:100%' rows = '3'></textarea>"
		       +"<br><a name='summitreply' href='#' onclick='return false;'>"+ac("提交")+"<a>"
                 +"<div>");
       $(container).append(win);
       
	   $(win).find("textarea:eq(0)").keyup(function(){
	          
			  $c = $(this).parent().find("span[name=captchareply]").val();
			  if($c == undefined){
			     $("span[name=captchareply]").remove();
				$(this).parent().find("a[name=summitreply]").after("<span name=\"captchareply\"></span>");	
			  }
			  
			  
			  var c = $("input[name=captchareply]").val();
			  if( c == undefined){
				  $("span[name=captchareply]").html("<img style=\"margin-top:5px\" src=\"/services/index/captcha?"+new Date().getTime()+"\"/><input name=\"captchareply\" >");
			  }
	   });
	   ...
	   if(content=="" || content == undefined || content.indexOf("大紀元將選取精彩讀者評論在全球報紙版面上刊登")>0
	       || content.indexOf("大纪元将选取精彩读者评论在全球报纸版面上刊登")>0)
	       {
	          alert(ac("請輸入內容"));
	          return false;
	       }
	   ...
	   var captcha = $("input[name=captchareply]").val();
	   
       $(this).closest("div[name=input]").remove();
	   
	   $.ajax({
	       url:commenturl+"/services/ajax/replycomment?aid="+tempid+"&ucid="+ucid+"&content="+content+"&captcha="+captcha+"&callback=?",
	       dataType:"jsonp",
	       success:function(data){
	          alert(data.message);
	       }
   ...
   $(html).find("a[name=flagbad]").click(function(){
	   var rowData = $(this).closest("div[name=row]").data("row");
	   var ucid = rowData.id;

	   $.ajax({
	       url:commenturl+"/services/ajax/flagbad?aid="+tempid+"&ucid="+ucid+"&callback=?",
	       dataType:"jsonp",
	       success:function(data){
	          alert(ac("已經提交，謝謝!"));
	       }
   ...
   $(html).find("a[name=digg]").click(function(){
	   var rowData = $(this).closest("div[name=row]").data("row");
	   var ucid = rowData.id;

	   $.ajax({
	       url:commenturl+"/services/ajax/digg?aid="+tempid+"&ucid="+ucid+"&callback=?",
	       dataType:"jsonp",
	       success:function(data){
	          alert(ac("已經提交，謝謝!"));
	       }
   ...
   populateChildren(id,ec,level+1,row.id)
}
...
function rowAge(time)
{
   ...
   if(year>0){
      ...
      age = ac("發表時間")+":&nbsp;"+hour + "&nbsp;"+ac("小時")+"&nbsp;"+minute+"&nbsp;"+ac("分鐘以前");
   }else if(minute>0){
      age = ac("發表時間")+":&nbsp;"+minute + "&nbsp;"+ac("分鐘")+"&nbsp;"+second+"&nbsp;"+ac("秒以前");
   }else{
