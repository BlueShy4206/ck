...
$(document.body).ready(function() {
	...
	$('.bumpbox').click(function(e){
		...
		var el = $('<div style="width:1px;height:1px;position:absolute;border:2px solid #303132;padding:4px; url(' + path + 'ajax-loader.gif) no-repeat center center;left:' + middleH + 'px;top:' + (middleV+s) + 'px;cursor:pointer;display:block;z-index:100000;-moz-border-radius:10px;-webkit-border-radius:10px;border-radius:10px;font-size:16px;color:#FF8F19;">IE的用戶在關閉影片時，如遇黑畫面請稍候</div>');

		var bg = $('<div style=";width:100%;height:100%;opacity:0.9;position:absolute;top:' + s + 'px;left:0;"></div>');
...
//								'coverart.jpg', 
						...
						var x = $('<iframe id="test" src="' + content + '" frameborder="0" style="overflow:auto;frameborder:0;width:' + maxwInnerDiv + 'px;height:' + maxhInnerDiv + 'px; frameborder="0" allowfullscreen"></iframe>');
						div.append(x);
					}
		...
		if(allBumps[nextID] != null){
			var nx = $('<a style="width:30px;height:20px;background:#000 url(' + path + 'next.jpg) no-repeat center center;position:absolute;right:10px;bottom:-25px;display:block;"></a>');
		...
		if(allBumps[prevID] != null){
			var nx2 = $('<a style="width:30px;height:20px;background:#000 url(' + path + 'prev.jpg) no-repeat center center;position:absolute;right:50px;bottom:-25px;display:block;"></a>');
