...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "forumPost")) {
  $insertSQL = sprintf("INSERT INTO forum (forum_title, forum_type, forum_img, forum_content, forum_date, forum_lastman, forum_lastdate, forum_username, forum_ip) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['forum_title'], "text"),
                       GetSQLValueString($_POST['forum_type'], "text"),
                       GetSQLValueString($_POST['forum_img'], "text"),
                       GetSQLValueString($_POST['forum_content'], "text"),
                       GetSQLValueString($_POST['forum_date'], "int"),
                       GetSQLValueString($_POST['forum_lastman'], "text"),
                       GetSQLValueString($_POST['forum_lastdate'], "int"),
                       GetSQLValueString($_POST['forum_username'], "text"),
                       GetSQLValueString($_POST['forum_ip'], "text"));
...
<div id="main">
  ...
  <form action="<?php echo $editFormAction; ?>" method="POST" name="forumPost" id="forumPost" onsubmit="YY_checkform('forumPost','forum_title','#q','0','請檢查標題','forum_content','5','1','請檢查留言內容');return document.MM_returnValue">
...
<option value="4.gif">求救</option>
 ...
 <img name="face" src="images/face/1.gif" alt="forum_type" />
    ...
    <input type="hidden" name="MM_insert" value="forumPost" />
  </form>
  <? }else{  //否則顯示另一個區塊?>
