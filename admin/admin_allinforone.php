...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	...
	try {  //下載 檔案
		if(is_file($file)){
			...
			echo "<script>javascript:alert(\"請選擇檔案\");</script>";//跳出錯誤訊息
			echo "<script>history.back()</script>";//回上一頁
			exit();
		}
    } catch(Exception $e) {
	...
	$sumFail = 0;
	$sumexist = 0;
	//echo "count sheetData:$sumSheet<br>";
   foreach($sheetData as $key => $col){
	...
	if($key != 1){
        foreach ($col as $colkey => $colvalue) {
        	...
        	//echo "key:$colkey:".$col[$colkey].", ";
	    }
	    mysql_select_db($database_conn_web, $conn_web);
	    $query_web_member = sprintf("SELECT id FROM forfirst WHERE id LIKE %s ORDER BY id DESC LIMIT 0,1", GetSQLValueString($col['E'], "text"));
	    ...
	    if($totalRows_web_member == 0){
	    	...
	    	$insertSQL = sprintf("INSERT INTO forfirst (id, password, seepass, name, school, Student_ID, Department, Grade, sex, chinese_level, chinese, social_level, social, math_level, math, physical_level, physical ,note ,note3 ,note2 ,note4) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
	    			GetSQLValueString($col['E'], "text"),  //id
	    			GetSQLValueString(md5($col['F']), "text"), //password
	    			GetSQLValueString($col['F'], "text"),  //seepass
	    			GetSQLValueString($col['K'], "text"),  //name
	    			GetSQLValueString($col['G'], "text"),  //school
	    			GetSQLValueString($col['L'], "text"),  //Student_ID
	    			GetSQLValueString($col['I'], "text"),  //Department
	    			GetSQLValueString($col['J'], "text"),  //Grade
	    			GetSQLValueString('', "text"),  //sex
	    				
	    			GetSQLValueString($col['V'], "text"),  //chinese_level
	    			GetSQLValueString($col['R'], "text"),  //chinese
	    			GetSQLValueString($col['W'], "text"),  //social_level
	    			GetSQLValueString($col['S'], "text"),  //social
	    			GetSQLValueString($col['X'], "text"),  //math_level
	    			GetSQLValueString($col['T'], "text"),  //math
	    			GetSQLValueString($col['Y'], "text"),  //physical_level
	    			GetSQLValueString($col['U'], "text"),  //physical
	    			GetSQLValueString($col['Z'], "text"),  //note_chinese
	    			GetSQLValueString($col['AA'], "text"),  //note_social
	    			GetSQLValueString($col['AB'], "text"),  //note_math
	    			GetSQLValueString($col['AC'], "text")  //note_physical
	    	...
	    	if($Result1){
	    		$sumSuccess++;
	    		$insertSQL2 = sprintf("INSERT INTO member (id, username, password, uname, eng_uname, uid, sex, birthday, email, orderPaper, phone, Area, cityarea, cuszip, cusadr, level, date, EForm_MK) VALUES (NULL, %s, %s, %s, '', '', '', '', '', 'N', '', '', '', '', '', 'member', '' ,'1')",
	    				GetSQLValueString($col['E'], "text"), //id
	    				GetSQLValueString(md5($col['F']), "text"), //password
	    				GetSQLValueString($col['K'], "text")  //name
	    		);
	    		...
	    		$Result2 = mysql_query($insertSQL2, $conn_web) or die(mysql_error());
	    	}
	    
	    	$memerr=$memerr.$col['E']."、";  //有新增的帳號
	    	...
	    	$sumexist++;
	    	//修改密碼所使用的機制 以不使用 但如果重會 只有帳號 將不會有動作 如有需要 請自行修改
   ...
   $_SESSION[sumSuccess]=$sumSuccess;
	  //echo "succes:$sumSuccess , fail:$sumFail, exist:$sumexist <br>";
