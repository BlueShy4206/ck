...
define("DESTINATION_FILE_FOLDER", "allin/");
...
for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
	for ($j = 1; $j <= $data->sheets[0]['numCols']; $j++) {
	 ...
	 $Ticket=substr(($score[14]),0,1).$_POST['times'].$_POST['year'];
...
$number=1;
	 ...
	 mysql_select_db($database_conn_web, $conn_web);
     $query_web_member = sprintf("SELECT id FROM member WHERE username LIKE %s ORDER BY id DESC LIMIT 0,1", GetSQLValueString($score[2], "text"));
	...
	if($score[3] != "" || $score[2] != "" || $totalRows_web_member == 0){
	if($totalRows_web_member == 0){	
	$insertSQL = sprintf("INSERT INTO member (username, password, uname, uid, sex, birthday, email, phone, Area, cityarea, cuszip, cusadr, level, date) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($score[2], "text"),
                       GetSQLValueString(md5($score[3]), "text"),
                       ...
                       GetSQLValueString($score[7], "date"),
					   GetSQLValueString($score[8], "text"),
					   GetSQLValueString($score[9], "text"),
					   GetSQLValueString(mb_convert_encoding($score[10],"utf-8","big5"), "text"),
                       GetSQLValueString(mb_convert_encoding($score[11],"utf-8","big5"), "text"),
					   GetSQLValueString($score[12], "text"),
					   GetSQLValueString(mb_convert_encoding($score[13],"utf-8","big5"), "text"),
  ...
  $Result1 = mysql_query($insertSQL, $conn_web) or die(mysql_error());
	 ...
	 $memerr=$memerr.$score[2]."、";
	 $_SESSION[memerr]=$memerr;
	}
		}
	
	    $nowtime=$_POST['times'].$_POST['year'];
		mysql_select_db($database_conn_web, $conn_web);
        $query_web_examinee = sprintf("SELECT * FROM examinee WHERE username = %s AND id LIKE %s ORDER BY DATE DESC", 
                  GetSQLValueString($score[2], "text"),
                  GetSQLValueString("%" . $nowtime . "%", "text"));
        ...
        $totalRows_web_examinee = mysql_num_rows($web_examinee);	
             ...
             if($score[2] != "" || $score[14] != ""){
			if($totalRows_web_examinee == 0){
  $insertSQL = sprintf("INSERT INTO examinee (id, username, uname, sex, birthday, per_id, email, phone, Area, cityarea, cuszip, cusadr, exarea, school, category, Grade, Student_ID, Department, status, date) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
					   ...
					   GetSQLValueString($score[12], "text"),
					   GetSQLValueString(mb_convert_encoding($score[13],"utf-8","big5"), "text"),
					   GetSQLValueString($score[14], "text"),
                       GetSQLValueString(mb_convert_encoding($score[15],"utf-8","big5"), "text"),
					   GetSQLValueString($score[16], "text"),
					   GetSQLValueString($score[17], "text"),
					   GetSQLValueString($score[18], "text"),
                       GetSQLValueString(mb_convert_encoding($score[19],"utf-8","big5"), "text"),
					   GetSQLValueString(mb_convert_encoding($score[20],"utf-8","big5"), "text"),
					   GetSQLValueString($_POST['date'], "date"));
