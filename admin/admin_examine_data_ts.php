...
// 		echo "qry_exareaNo=".$qry_exareaNo."<br>";
		...
		$sql_s="SELECT examinee.*, allguide.note as exam_school, examinee_pic.*, check_review.* FROM examinee, allguide, examinee_pic, check_review WHERE allguide.up_no='EA2'
					...
					AND examinee.id LIKE '_". $_POST['times'] . $_POST['startyear'] ."%' AND examinee.apply_mk = '1' AND examinee.no = examinee_pic.examinee_no AND examinee.no = check_review.examinee_sn ";
		if($_POST['allow'] != ""){
      if	($_POST['allow'] == "N"){
...
// 			$objPHPExcel->getActiveSheet()->setCellValue('AM'.$i ,$ZLA); //備取順序
      ...
      //匯入 照片1~5,特殊考生,戶口名簿 END
        ...
        if($row['final_trial']=='OK'){$row['final_trial']= '通過';}else if($row['final_trial']=='NO'){$row['final_trial']= '不通過';}
