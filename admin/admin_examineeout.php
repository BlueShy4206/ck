...
//PHPExcel 套件
		...
		$query_web_Recordset2 = "SELECT * FROM examinee WHERE id LIKE '%". $_POST['times']. $_POST['startyear'] ."%' AND allow ='Y' AND id_number is not null ORDER BY exarea,exarea_note,id";
		...
		while($row = mysql_fetch_array($result))
		{
			...
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$i ,substr($row['per_id'],4,6)); //密碼
			...
			$objPHPExcel->getActiveSheet()->setCellValue('N'.$i ,$row['exarea'].'_'.$row['exarea_note']); //考區
		...
		//寬度設定 右邊 為 自動列寬 setAutoSize(true) 不建議使用  
		 ...
		 require_once('./PHPExcel/Classes/PHPExcel/Writer/Excel5.php');
				...
				$objWriter->save(str_replace('.php', '.xls',  './downloadFile/'.date('Y-n-j').'_stuteach.xls'));
                $downloadFile = './downloadFile/'.date('Y-n-j').'_stuteach.xls';
