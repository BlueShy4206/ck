...
function downloadxls(){
		...
		while($row2 = mysql_fetch_array($result_n))
		{
			...
			$objPHPExcel->getActiveSheet()->setCellValue('O1' ,"就讀學校"); //(就讀/任職)學校
			$objPHPExcel->getActiveSheet()->setCellValue('P1' ,"年級"); //年級
			$objPHPExcel->getActiveSheet()->setCellValue('Q1' ,"系所"); //系所
			$objPHPExcel->getActiveSheet()->setCellValue('R1' ,"學號"); //學號

			$objPHPExcel->getActiveSheet()->setCellValue('S1' ,"評量考區"); //評量考區
			$objPHPExcel->getActiveSheet()->setCellValue('T1' ,"評量日期"); //評量日期
			$objPHPExcel->getActiveSheet()->setCellValue('U1' ,"考科 \n 1國 2數 \n 3社 4自"); //考科

			$objPHPExcel->getActiveSheet()->setCellValue('V1' ,"緊急聯絡人 姓名"); //緊急聯絡人
			$objPHPExcel->getActiveSheet()->setCellValue('W1' ,"緊急聯絡人 電話"); //緊急聯絡人 電話
			/*
...
// 			$objPHPExcel->getActiveSheet()->setCellValue('AM1' ,"錄取順序"); //錄取順序
			$objPHPExcel->getActiveSheet()->setCellValue('AM1' ,"審核備註"); //審核備註,add by coway 2016.10.4
			$objPHPExcel->getActiveSheet()->setCellValue('AN1' ,"身份別"); //身份別,add by coway 2016.10.4
			$objPHPExcel->getActiveSheet()->setCellValue('AO1' ,"流水號"); //流水號,add by coway 2016.10.4
			$objPHPExcel->getActiveSheet()->setCellValue('AP1' ,"帳號"); //帳號,add by coway 2016.10.4
			$objPHPExcel->getActiveSheet()->setCellValue('AQ1' ,"身分證正面");
      $objPHPExcel->getActiveSheet()->setCellValue('AR1' ,"身分證反面");
      $objPHPExcel->getActiveSheet()->setCellValue('AS1' ,"修畢師資職前教育證明書");
      $objPHPExcel->getActiveSheet()->setCellValue('AT1' ,"實習學生證");
      $objPHPExcel->getActiveSheet()->setCellValue('AU1' ,"學生證正面");
      ...
      $objPHPExcel->getActiveSheet()->setCellValue('AY1' ,"戶口名簿");
		...
		//撈取資料庫
		$qry_exarea =  Explode(",", $_POST['exarea'])[0];
		$qry_exareaNo =  Explode(",", $_POST['exarea'])[1];
// 		echo "qry_exarea=".$qry_exarea."<br>";
// 		echo "qry_exareaNo=".$qry_exareaNo."<br>";
		//$sql_s = "SELECT * FROM examinee WHERE id LIKE '%". $_POST['times'] . $_POST['startyear'] ."%' ORDER BY exarea,exarea_note,no ASC";
		...
		if($_POST['allow'] != ""){	$sql_s = $sql_s ."AND examinee.allow='".$_POST['allow']."' ";//add by coway 2017.5.9
		}
		if($qry_exarea != ""){	$sql_s = $sql_s ."AND examinee.exarea = '".$qry_exarea."' AND examinee.exarea_note = '".$qry_exareaNo."' ";//add by coway 2017.5.9
		}

      $sql_s = $sql_s ."ORDER BY examinee.date ASC";
		// $sql_s = $sql_s ."ORDER BY exarea,exarea_note,examinee.no ASC"; //AND examinee.allow='Y' AND allguide.no in ('S') AND exarea='Central', AND allguide.no='C' AND allguide.data2='22' AND allow='Y'
// 		echo $sql_s;
// 		die();
...
// 			$Ar_city_cu = $row['Area'].$row['cityarea'].$row['cusadr']; //update by coway 2016.10.12
			$Ar_city_cu = $row['cusadr'];
			...
			$objPHPExcel->getActiveSheet()->setCellValue('P'.$i ,$row['Grade']); //年級
			$objPHPExcel->getActiveSheet()->setCellValue('Q'.$i ,$row['Department']); //系所
			$objPHPExcel->getActiveSheet()->setCellValue('R'.$i ,$row['Student_ID']); //學號
			$objPHPExcel->getActiveSheet()->setCellValue('S'.$i ,$row['exam_school']); //考區
			$objPHPExcel->getActiveSheet()->setCellValue('T'.$i ,$row['exarea_date']); //日期
			$objPHPExcel->getActiveSheet()->setCellValue('U'.$i ,$row['category']); //考科

			$objPHPExcel->getActiveSheet()->setCellValue('V'.$i ,$row['contact']); //緊急聯絡人
			$objPHPExcel->getActiveSheet()->setCellValue('W'.$i ,$row['contact_ph']); //緊急聯絡人 電話
			/*
			...
			*/
			...
			$objPHPExcel->getActiveSheet()->setCellValue('AE'.$i ,$row['id_number']);//id_number
...
// 			$objPHPExcel->getActiveSheet()->setCellValue('AM'.$i ,$ZLA); //備取順序
			...
			$objPHPExcel->getActiveSheet()->setCellValue('AN'.$i ,$row['cert_no']); //身份別,add by coway 2016.10.5
			$objPHPExcel->getActiveSheet()->setCellValue('AO'.$i ,$row['cert_id']); //流水號,add by coway 2016.10.5
			$objPHPExcel->getActiveSheet()->setCellValue('AP'.$i ,$row['username']); //帳號,add by coway 2016.10.5
        ...
        // echo "<pre>";
        // print_r($row);
        // echo "<script>console.log( $row['pic1_name']);</script>";
        // die();
      ...
      //5
      if(is_file('../images/smallPic/id_check/'.$row['pic5_name'])){
				$objDrawing = new PHPExcel_Worksheet_Drawing();
				$objDrawing->setName($row['pic5_name']);
                                $objDrawing->setDescription($row['pic5_name']);
                                $objDrawing->setPath('../images/smallPic/id_check/'.$row['pic5_name']);
                                $objDrawing->setHeight(400);
				$objDrawing->setWidth(320);
				//$objDrawing->setWidthAndHeight(80,100);
				$objDrawing->setResizeProportional(true);
     ...
     //戶口名簿
     if(is_file('../images/smallPic/id_check/'.$row['rename_pic_name'])){
       $objDrawing = new PHPExcel_Worksheet_Drawing();
       $objDrawing->setName($row['rename_pic_name']);
                               $objDrawing->setDescription($row['rename_pic_name']);
                               $objDrawing->setPath('../images/smallPic/id_check/'.$row['rename_pic_name']);
                               $objDrawing->setHeight(100);
       $objDrawing->setWidth(80);
       //$objDrawing->setWidthAndHeight(80,100);
       $objDrawing->setResizeProportional(true);
		...
		//寬度設定 右邊 為 自動列寬 setAutoSize(true) 不建議使用
		 ...
		 $objPHPExcel->getActiveSheet()->getColumnDimension('AN')->setWidth(16);//身份別,add by coway 2016.10.5
		 $objPHPExcel->getActiveSheet()->getColumnDimension('AO')->setWidth(16);//流水號,add by coway 2016.10.5
		 $objPHPExcel->getActiveSheet()->getColumnDimension('AP')->setWidth(16);//帳號,add by coway 2016.10.5
...
//撈取資料庫 (考場) ,add by coway 2017.5.9
$sql_exarea = "SELECT no , nm, note,data1,data2 FROM allguide WHERE up_no = 'EA2'";
      ...
      <tr>
        ...
        <td width="305" align="left" class="board_add">
       		...
       		<?php while($row_exarea = mysql_fetch_array($result_exarea)){
       				echo "<option value=".$row_exarea['nm'].','.$row_exarea['data2'].">$row_exarea[note]$row_exarea[data1]</option>";
       			}
       		?>
       		</select> </td>
      </tr>
