...
$query_web_member = sprintf("SELECT * FROM member WHERE (username LIKE %s or uname LIKE %s) ORDER BY id ASC", GetSQLValueString($colname_web_search, "text"), GetSQLValueString($colname_web_search, "text"));
...
$row_web_member = mysql_fetch_assoc($web_member);
	...
	mysql_select_db($database_conn_web, $conn_web);
$query_web_member = "SELECT * FROM member ORDER BY id ASC";
$query_limit_web_member = sprintf("%s LIMIT %d, %d", $query_web_member, $startRow_web_member, $maxRows_web_member);
$web_member = mysql_query($query_limit_web_member, $conn_web) or die(mysql_error());
$row_web_member = mysql_fetch_assoc($web_member);
	}
  ...
  <div id="admin_main2">
   <?php if($row_web_member == 0){ ?>
      ...
      <tr>
        ...
        <td width="416" align="left" background="../images/board04.gif">       <form id="Search" name="Search" method="get" action="admin_member_search.php">
      <label>
    ...
    <table width="555" border="0" cellspacing="0" cellpadding="0">
      <?php do { ?>
        <tr>
          ...
          <td width="150" align="center" class="board_add">[ <a href="admin_memberUpdate.php?id=<?php echo $row_web_member['id']; ?>">編輯</a> ]&nbsp; [ <a href="admin_memberDel.php?id=<?php echo $row_web_member['id']; ?>&amp;delSure=1" onclick="tfm_confirmLink('確定刪除本資料?');return document.MM_returnValue">刪除</a> ]
          &nbsp;[<a href="<?php echo "$MailHost/EasyMVC/Home/queryList/$row_web_member[id]"; ?>" onclick="">驗証</a> ]</td>
        </tr>
