...
// // 		alert('uname='+uname);
// // 		alert('uname_len='+uname.length);
// 		$ . ajax ( {
// 			type : "POST" ,
// 			url : "process.php" ,
// 			data : { engNameForm2 : engNameForm2 ,uname : uname } ,
// 			async : false ,
// 			success : function ( data ) {
// 				response = data ;
// 			}
...
// 	} ) ;
	...
	$.validator.addMethod("engNameForm", function(value, element) {
		var a = new Array('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'X', 'Y', 'W', 'Z', 'I', 'O');
	...
	$.validator.addMethod("engNameForm2", function(value, element) {
		var a = new Array('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'X', 'Y', 'W', 'Z', 'I', 'O');
		var uname = $ ( '#uname' ) . val ( ) ;//中文姓名
		if(uname.length > 2)//判斷若中文名字三個字時，需輸入英文名字中間用空白隔開 ,add by coway 2016.8.5
		{
		...
		if(d == value.charAt(0)) {
			...
			if(words[1] != null){
				d2 = words[1].charAt(0).toUpperCase();
				//console.log("d2:"+d2);
				if(d2 == words[1].charAt(0)) {
		...
		$("#signupForm").validate({
			rules: {
				...
				Lastname: {
					required: true,
					engNameForm2: true,
// 					uniqueengNameForm2: true//add by coway
				},
				//eng_uname: "required",
				username: {
					required: true,
      ...
      <table width="540" border="0" cellspacing="0" cellpadding="2">
        ...
        <tr>
          ...
          <!-- <label for="eng_uname"><input id="eng_uname" name="eng_uname" type="text"></label> -->
          	姓(Last name)<label for="Firstname"><input id="Firstname" name="Firstname" type="text" size="10" ></label>
        ...
        <tr>
		  ...
		  <input type="button" value="Cal" onclick="displayCalendar(birthday,'yyyy-mm-dd',this)"> 格式為：YYYY-MM-DD
          <span class="font_red">* </span>          </td>
        </td>
        </tr>
        <tr>
